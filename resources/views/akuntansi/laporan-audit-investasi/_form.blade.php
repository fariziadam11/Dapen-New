{{-- Form fields for Laporan Audit Investasi --}}
<div class="row">
...
<div class="col-md-6 mb-3">
    ...
    <textarea name="judul" class="form-control @error('judul') is-invalid @enderror" rows="3" >{{ old('judul', $record->judul ?? '') }}</textarea>
    @error('judul')<div class="invalid-feedback">{{ $message }}</div>@enderror
</div>
<div class="col-md-6 mb-3">
    <label class="form-label">Nama KAP </label>
    <input type="text" name="nama_kap" class="form-control @error('nama_kap') is-invalid @enderror" value="{{ old('nama_kap', $record->nama_kap ?? '') }}" >
    @error('nama_kap')<div class="invalid-feedback">{{ $message }}</div>@enderror
</div>
