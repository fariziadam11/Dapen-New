...
<div class="row">
    ...
    <input type="text" name="masa" class="form-control @error('masa') is-invalid @enderror" value="{{ old('masa', $record->masa ?? '') }}" >
    @error('masa')<div class="invalid-feedback">{{ $message }}</div>@enderror
</div>
<div class="col-md-6 mb-3">
    <label class="form-label">Tahun </label>
    <input type="number" name="tahun" class="form-control @error('tahun') is-invalid @enderror" value="{{ old('tahun', $record->tahun ?? '') }}" step="0.01" >
    @error('tahun')<div class="invalid-feedback">{{ $message }}</div>@enderror
</div>
<div class="col-md-6 mb-3">
    <label class="form-label">Uraian </label>
    <input type="text" name="uraian" class="form-control @error('uraian') is-invalid @enderror" value="{{ old('uraian', $record->uraian ?? '') }}" >
