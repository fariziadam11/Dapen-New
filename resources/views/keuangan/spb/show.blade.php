...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('keuangan.spb.index') }}">SPB</a></li>
    <li class="breadcrumb-item active">Detail</li>
@endsection
@section('content')
    <div class="page-header mb-4">
        <h1 class="page-title">Detail SPB</h1>
    </div>
    <div class="card">
        <div class="card-body">
            <table class="table table-bordered">
                <tr>
                    <th width="200">Surat Bayar</th>
                    <td>{{ $record->suratBayar->nomor_surat_bayar ?? '-' }} @if ($record->suratBayar)
                            ({{ $record->suratBayar->tanggal_surat_bayar ? date('d/m/Y', strtotime($record->suratBayar->tanggal_surat_bayar)) : '' }})
                        @endif
                    </td>
                </tr>
                <tr>
                    <th width="200">Tanggal SPB</th>
                    <td>{{ $record->tanggal_spb ? date('d F Y', strtotime($record->tanggal_spb)) : '-' }}</td>
                </tr>
                <tr>
                    <th width="200">Nomor</th>
                    <td>{{ $record->nomor_spb ?? '-' }}</td>
                </tr>
                <tr>
                    <th width="200">Nominal</th>
                    <td>{{ number_format($record->nominal ?? 0, 0, ',', '.') }}</td>
                </tr>
                <tr>
