            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $items->firstItem() + $i }}</td>
                            <td>{{ $item->tahun ?? '-' }}</td>
                            <td>{{ $item->bulan ?? '-' }}</td>
                            <td>@if($item->file_name)<div class="btn-group btn-group-sm"><button onclick="previewFile('{{ route('logistik.pelaporan-prbc.preview', $item->id) }}', '{{ $item->file_name }}')" class="btn btn-outline-primary" title="Preview"><i class="bi bi-eye"></i></button><a href="{{ route('logistik.pelaporan-prbc.download', $item->id) }}" class="btn btn-outline-success" title="Download"><i class="bi bi-download"></i></a></div>@else<span class="text-muted">-</span>@endif</td>
                    <td>
                        <div class="btn-group btn-group-sm">
                            ...
                            <form action="{{ route('logistik.pelaporan-prbc.destroy', $item->id) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-outline-danger"><i class="bi bi-trash"></i></button></form>
                        </div>
                    </td>
                </tr>
                @empty
