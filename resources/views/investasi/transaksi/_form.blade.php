...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="type" class="form-select @error('type') is-invalid @enderror" required>
            ...
            <option value="1" {{ old('type', $record->type ?? '') == 1 ? 'selected' : '' }}>Penjualan</option>
            <option value="2" {{ old('type', $record->type ?? '') == 2 ? 'selected' : '' }}>Pembelian</option>
        </select>
        @error('type')
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="date" name="tgl_transaksi" class="form-control @error('tgl_transaksi') is-invalid @enderror"
            value="{{ old('tgl_transaksi', isset($record) && $record->tgl_transaksi ? $record->tgl_transaksi->format('Y-m-d') : '') }}">
        @error('tgl_transaksi')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="saham" class="form-control @error('saham') is-invalid @enderror"
            value="{{ old('saham', $record->saham ?? '') }}">
        @error('saham')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="broker" class="form-control @error('broker') is-invalid @enderror"
            value="{{ old('broker', $record->broker ?? '') }}">
        @error('broker')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
