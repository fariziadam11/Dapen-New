<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="kode_jabatan" class="form-control @error('kode_jabatan') is-invalid @enderror" value="{{ old('kode_jabatan', $record->kode_jabatan ?? '') }}" required>
        @error('kode_jabatan')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6 mb-3">
        <label class="form-label">Nama Jabatan <span class="text-danger">*</span></label>
        <input type="text" name="nama_jabatan" class="form-control @error('nama_jabatan') is-invalid @enderror" value="{{ old('nama_jabatan', $record->nama_jabatan ?? '') }}" required>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="id_department" class="form-select @error('id_department') is-invalid @enderror">
            ...
            @foreach($departments ?? [] as $dept)
                <option value="{{ $dept->id }}" {{ old('id_department', $record->id_department ?? '') == $dept->id ? 'selected' : '' }}>{{ $dept->nama_department }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="level_jabatan" class="form-select @error('level_jabatan') is-invalid @enderror">
            ...
            <option value="1" {{ old('level_jabatan', $record->level_jabatan ?? '') == 1 ? 'selected' : '' }}>Staff</option>
            <option value="2" {{ old('level_jabatan', $record->level_jabatan ?? '') == 2 ? 'selected' : '' }}>Supervisor</option>
            <option value="3" {{ old('level_jabatan', $record->level_jabatan ?? '') == 3 ? 'selected' : '' }}>Manager</option>
            <option value="4" {{ old('level_jabatan', $record->level_jabatan ?? '') == 4 ? 'selected' : '' }}>Director</option>
        </select>
