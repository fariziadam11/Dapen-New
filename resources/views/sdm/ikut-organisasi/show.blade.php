...
@endsection
@section('content')
<div class="page-header mb-4"><h1 class="page-title">Detail Ikut Organisasi</h1></div>
<div class="card">
    <div class="card-body">
        ...
        <tr><th>File</th><td>@if($record->file_name)<a href="{{ route('sdm.ikut-organisasi.download', $record->id) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-download"></i> {{ $record->file_name }}</a>@else - @endif</td></tr>
        ...
        <div class="d-flex justify-content-between">
            ...
            <div>
                <a href="{{ route('sdm.ikut-organisasi.edit', $record->id) }}" class="btn btn-warning"><i class="bi bi-pencil"></i> Edit</a>
                <form action="{{ route('sdm.ikut-organisasi.destroy', $record->id) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-danger"><i class="bi bi-trash"></i> Hapus</button></form>
            </div>
        </div>
    </div>
</div>
