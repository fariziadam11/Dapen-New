...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('sekretariat.laporan.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>
            Tambah</a>
    </div>
    <div class="card mb-4">
        <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($item->file_name)
                                    <div class="btn-group btn-group-sm"><button
                                            onclick="previewFile('{{ route('sekretariat.laporan.preview', $item->id) }}', '{{ $item->file_name }}')"
                                            class="btn btn-outline-primary" title="Preview"><i
                                                class="bi bi-eye"></i></button><a
                                            href="{{ route('sekretariat.laporan.download', $item->id) }}"
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('sekretariat.laporan.show', $item->id) }}"
                                        class="btn btn-outline-primary"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('sekretariat.laporan.edit', $item->id) }}"
                                        class="btn btn-outline-warning"><i class="bi bi-pencil"></i></a>
                                    <form action="{{ route('sekretariat.laporan.destroy', $item->id) }}" method="POST"
