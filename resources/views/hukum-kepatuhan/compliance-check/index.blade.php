...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('hukum-kepatuhan.compliance-check.create') }}" class="btn btn-primary"><i
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($item->file_name)
                                    <div class="btn-group btn-group-sm"><button
                                            onclick="previewFile('{{ route('hukum-kepatuhan.compliance-check.preview', $item->id) }}', '{{ $item->file_name }}')"
                                            class="btn btn-outline-primary" title="Preview"><i
                                                class="bi bi-eye"></i></button><a
                                            href="{{ route('hukum-kepatuhan.compliance-check.download', $item->id) }}"
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('hukum-kepatuhan.compliance-check.show', $item->id) }}"
                                        class="btn btn-outline-primary"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('hukum-kepatuhan.compliance-check.edit', $item->id) }}"
                                        class="btn btn-outline-warning"><i class="bi bi-pencil"></i></a>
                                    <form action="{{ route('hukum-kepatuhan.compliance-check.destroy', $item->id) }}"
