...
@section('content')
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('hukum-kepatuhan.putusan.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>
            Tambah</a>
    </div>
    <div class="card mb-4">
        <div class="card-body">
                ...
                <tbody>
                    @forelse($items as $i => $item)
                        <tr>
                            ...
                            <td>{{ $item->nomor ?? '-' }}</td>
                            <td>{{ $item->klarifikasi ?? '-' }}</td>
                            <td>{{ $item->tingkat_pengadilan ?? '-' }}</td>
                            <td>{{ $item->status ?? '-' }}</td>
                            <td>{{ $item->pihak ?? '-' }}</td>
                            <td>
                                @if ($item->file_name)
                                    <div class="btn-group btn-group-sm"><button
                                            onclick="previewFile('{{ route('hukum-kepatuhan.putusan.preview', $item->id) }}', '{{ $item->file_name }}')"
                                            class="btn btn-outline-primary" title="Preview"><i
                                                class="bi bi-eye"></i></button><a
                                            href="{{ route('hukum-kepatuhan.putusan.download', $item->id) }}"
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('hukum-kepatuhan.putusan.show', $item->id) }}"
                                        class="btn btn-outline-primary"><i class="bi bi-eye"></i></a>
                                    <a href="{{ route('hukum-kepatuhan.putusan.edit', $item->id) }}"
                                        class="btn btn-outline-warning"><i class="bi bi-pencil"></i></a>
                                    <form action="{{ route('hukum-kepatuhan.putusan.destroy', $item->id) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center py-4 text-muted">Belum ada data</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
